...
def reset_searches():
    ...
    # Generate a list with random numbers and include 93, then sort it for binary search
    numbers = random.sample(range(1, 150), 31) + [93]
...
def update_searches(dt):
    ...
    if not binary_found and binary_left <= binary_right:
        binary_mid = (binary_left + binary_right) // 2
        if numbers[binary_mid] == 93:
            binary_found = True
        elif numbers[binary_mid] < 93:
...
def on_draw():
    ...
    for i, number in enumerate(numbers):
        ...
        color_linear = (0, 255, 0) if linear_found and i == linear_index - 1 else (200, 0, 255) if i == linear_index else (200, 200, 200)
        ...
        color_binary = (0, 255, 0) if binary_found and i == binary_mid else (200, 0, 255) if binary_left <= i <= binary_right else (200, 200, 200)
